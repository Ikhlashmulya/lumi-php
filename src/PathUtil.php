...
class PathUtil {
    public static function getParamNames(string $rawPath) {
        preg_match_all('/\{([a-zA-Z0-9]+)\}/', $rawPath, $paramNames);
        return $paramNames[1];
    }

    public static function toRegexPath(string $rawPath) {
        return preg_replace('/\{([a-zA-Z0-9]+)\}/', '([a-zA-Z0-9]+)', $rawPath);
    }

    public static function isMatch(string $rawPath, string $uri) {
